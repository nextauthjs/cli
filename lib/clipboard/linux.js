...
const findClipboardProgram = () => {
  if (existsSync("/bin/wl-copy")) {
    return {
      cmd: "/bin/wl-copy",
      env: ["WAYLAND_DISPLAY=wayland-1", ...env],
    }
  }
  if (existsSync("/bin/xclip")) {
    return {
      cmd: "/bin/xclip",
      env: [...env],
    }
  }
  if (existsSync("/bin/xsel")) {
    return {
      cmd: "/bin/xsel",
    }
  }
...
const clipboard = {
  copy: (options) => {
    ...
    if (!program.cmd) {
      throw new Error("No clipboard program found for your platform")
    }
    let envVarString = ""
    if (program.env) {
